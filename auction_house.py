...
def get_ah_data(realm_id: int=57) -> set:
    ...
    for ah_item_id in db["auction_house_data"].find({}).distinct('item.id'):
        print(f"Getting item for {ah_item_id}")
        if not db["items"].find_one({'id': ah_item_id}):
            item = blizzard.game_item(ah_item_id)
            item["date_time"] = current_time
    ...
    for item_id in db["items"].find({}).distinct('id'):
        ...
        if item:
            ...
            image_b64 = base64.b64encode(r.content)
